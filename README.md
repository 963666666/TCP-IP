...
------
    ...
    *电子邮件
        MIME  
            *MIME基本上由首部和正文（数据）两部分组成。首部不能是空行，一旦出现空行，其后的部分将被  
            视为正文数据。如果MIME首部的“Content-Type”中指定“Multipart/Mixed”，并以“bounday=”  
            后面字符作为分隔符，那么可以将多个MIME消息组合成为一个MIME消息。这叫做multipart。即，  
            各个部分都由MIME首部和正文（数据）组成。
    *POP
...
其他应用层协议
------
    *呼叫控制  主要采用H.323与SIP协议。此外还需要RTP协议（结合多媒体数据本身的特性进行传输的一种  
    协议）和压缩技术（在网络上传输音频、视频等大型多媒体数据时进行压缩）的支持。
    *H.323是由ITU开发用于IP网上传输音频、视频的一种协议。定义了4个组件。终端（用户终端）、网关  
    （吸收用户数据压缩数据的不一致性）、网闸（电话本管理、呼叫管理）以及多点控制单元（允许多个终端  
    同时使用）  
    *SIP  终端之间进行多媒体通信时，需要有事先解析对方地址、呼出对方号码并对所要传输的媒体信息进行  
    处理等功能。此外，还需要有中断回话和数据转发的功能。这些功能（呼叫控制与信令）都被统一与SIP协议。
    
    *RTP  UDP不是一种可靠性传输协议。因此有可能发生丢包或乱序等现象。因此采用UDP实现实时的多媒体通
    信需要附加一个表示报文顺序的序列号字段，还需要对报文发送时间进行管理。这些正是RTP的主要职责。
    *RTP为每个报文附加时间戳和序列号。接收报文的引用，根据时间戳决定数据重构的时机。序列号则根据每  
    发出一次报文加一的原则进行累加。RTP使用这个序列号对同一时间戳的数据进行排序，掌握是否有丢包的  
    情况发生。
    *RTCP是辅助RTP的一种协议。通过丢包率等线路质量的管理，对RTP的数据传送率进行控制。    
    
	*数据压缩技术  通过有效的压缩可以大量减少音频和视频数据的大小。
	*P2P
	*LDAP常被用于认证管理和资源管理中。
